...
@section('content')
        <nav class="navbar navbar-main navbar-expand-lg  px-0 mx-4 shadow-none border-radius-xl z-index-sticky "
            ...
            <div class="container-fluid py-1 px-3">
                @include('layouts.navbars.auth.topnav', ['title' => 'New Ticket'])
        ...
        <div class="container-fluid py-4">
            <div class="row">
                <form method="POST" action="{{ route('support-tickets-create-store') }}" enctype="multipart/form-data">
                @csrf
                    <div class="col-lg-9 col-12 mx-auto">
                        <div class="card card-body mt-4">                
                            <h6 class="mb-0">New Ticket</h6>
                            <p class="text-sm mb-0">New Ticket Create</p>
                            ...
                            <select class="form-control" name="userId"
                                ...
                                @foreach (App\Models\User::whereNull('role_id')->get() as $user_val)
                                    <option value="{{$user_val->id}}">{{$user_val->firstname}}</option>
                                @endforeach
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" class="form-control" id="projectName">
                            @error('title')
                                <p class='text-danger text-xs pt-1'> {{ $message }} </p>
                            @enderror

                            <label class="mt-4">Ticket Description</label>
                            <textarea name="message" rows="2" class="w-100 form-control">{{ old('message') }}</textarea>
                            @error('message')
                                <p class='text-danger text-xs pt-1'> {{ $message }} </p>
                            @enderror
